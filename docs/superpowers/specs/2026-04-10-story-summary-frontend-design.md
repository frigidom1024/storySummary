...
**布局：**
- **多行并行时间线**：按 `thread_id` 分行展示，每条叙事线独立一行
- 同一 `timeline_order` 的节点按行对齐（横向看同一时刻）
- 每行上方标注 `thread_name`
...
> **后端调试用途**：通过多行并行展示，可直观看到不同叙事线在同一 timeline_order 下的分布情况。
...
| thread_id | string | 叙事线ID（如"main"、"zhang"） | 筛选/卡片标签 |
| thread_name | string | 叙事线名称（如"张医生的主线"） | 卡片标签 |
...
interface NodeFilter {
  ...
  visibleThreads: string[]; // 要显示的叙事线 ID 列表（空数组=全部显示）
}
```
